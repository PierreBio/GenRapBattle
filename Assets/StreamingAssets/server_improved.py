...
print("CUDA (GPU support) est disponible dans PyTorch")
print("Nombre de GPU disponibles:", torch.cuda.device_count())

# Liste des GPU disponibles et leurs informations
...
try:
    ...
    while True:
        ...
        generated_words = generate(words[len(words)-1], n_words=30, temperature=0.5, topk=7)
        words.append(generated_words)
        new_words.append(generated_words)
        bytes_to_send = ' '.join(new_words).encode('utf-8')
